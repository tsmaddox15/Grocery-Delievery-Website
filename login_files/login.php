          ...
          else  //passwords match
          {
            if (existing_username($db, $username)) {
              ...
              if (addUser($db, $username, $encrypt_password)){
                addCustomer($db, $username);  
                updateFirst($db, $username, $_POST['firstName']);
                updateLast($db, $username, $_POST['lastName']);
                 updateAddress($db, $username,  $_POST['address']);
                $_SESSION['message'] = 'Your account has been created and you are being logged in';
                $_SESSION['current_user'] = $_POST['username'];
                header('Location: login_message.php');
              }
            else {
  ...
  <body>      
    ...
    <main>              
        <form action="" method="post">
          ...
          <?php
              echo "<input type='text' pattern='^\w+@[a-zA-Z_]+?\.[a-zA-Z]{2,3}$'  required title='Please enter a valid email' name='username' value=$username_check>"; 
          ...
          <?php
             if ($_SESSION['type'] == "new"){
                ...
                echo ' <label for="firstName" class="profile">First Name</label>';
                echo '  <input type="text" name="firstName" required="required" value =""';
                echo '     <label for="lastName" class="profile">Last Name</label>';
                echo '<input type="text" name="lastName" required="required" value =""<br />';
                echo ' <label for="password" class="profile">Address</label>';
                echo '<input type="text"  pattern="^\d+\s[A-z]+\s[A-z]+"  required title="Please enter a vaild address"  name="address"value =""<br />';
             }
           ?>
